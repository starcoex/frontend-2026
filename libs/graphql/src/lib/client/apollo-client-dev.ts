...
// ✅ 신규: 채팅 서비스 엔드포인트
const getChatGraphQLUri = (isDev: boolean): string =>
  isDev ? '/chat/graphql' : 'http://localhost:4104/graphql';

const getChatWsUri = (isDev: boolean): string =>
  isDev
    ? `ws://${window.location.host}/chat/ws` // ✅ Vite 프록시 경유
    : 'ws://localhost:4104/graphql';
...
// ============================================================================
// ✅ 신규: Chat ApolloClient (Subscription 전용)
// ============================================================================

let chatClient: ApolloClient | null = null;

/**
 * 채팅 서비스 전용 ApolloClient
 * - HTTP: Query / Mutation
 * - WebSocket: Subscription (messageAdded, messageDeleted)
 */
export const createChatApolloClient = (): ApolloClient => {
  const isDev = process.env.NODE_ENV === 'development';

  // HTTP Link (Query / Mutation)
  const chatHttpLink = createHttpLink({
    uri: getChatGraphQLUri(isDev),
    credentials: 'include',
  });
  ...
  const chatWsLink = new GraphQLWsLink(
    createClient({
      url: getChatWsUri(isDev),
      connectionParams: () => ({
        // Cookie 기반 인증 — 브라우저가 자동으로 쿠키를 포함
      ...
      on: {
        connected: () => console.info('[ChatWS] WebSocket 연결됨'),
        closed: () => console.info('[ChatWS] WebSocket 연결 해제'),
        error: (err) => console.error('[ChatWS] WebSocket 오류:', err),
      },
    })
  );
...
export const initializeChatApolloClient = (): ApolloClient => {
  if (!chatClient) {
    chatClient = createChatApolloClient();
    console.log('✅ Chat Apollo Client initialized');
  }
  return chatClient;
};

export const getChatApolloClient = (): ApolloClient => {
  if (!chatClient) {
    throw new Error('Chat Apollo Client가 초기화되지 않았습니다.');
  }
  return chatClient;
};

export const resetChatApolloClient = (): void => {
  chatClient?.stop();
  chatClient = null;
};
