...
export const JobApplyForm: React.FC<JobApplyFormProps> = ({
  ...
  const [profileImage, setProfileImage] = useState<string | null>(
    // 수정 모드: 기존 PROFILE_IMAGE fileUrl 표시
    profile?.files?.find((f) => f.fileType === 'PROFILE_IMAGE')?.fileUrl ?? null
  );
  ...
  >(
    // 수정 모드: 기존 ATTACHMENT 파일 목록 복원
    profile?.files
      ?.filter((f) => f.fileType === 'ATTACHMENT')
      .map((f) => ({
        name: f.fileName ?? '첨부파일',
        fileUrl: f.fileUrl,
  ...
  useEffect(() => {
    if (currentUser?.id) {
      getUserAddresses({ page: 1, limit: 10 });
    }
  ...
  const handleProfileImage = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // 실제 업로드
    try {
      const res = await uploadMedia([file], currentUser.id, 'PROFILE_IMAGE');
      if (res.success && res.data) {
        const response = res.data as any;
        const fileUrl: string =
          response.files?.[0]?.fileUrl ?? response.fileUrl;

        if (fileUrl) {
          // 기존 PROFILE_IMAGE 제거 후 새로 추가
          setUploadedFiles((prev) => [
            ...prev.filter((f) => f.fileType !== 'PROFILE_IMAGE'),
            {
              name: file.name,
              fileUrl,
              fileSize: file.size,
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (res.success && res.data) {
        ...
        // add-product-form.tsx 와 동일한 패턴
        const fileUrls: string[] = response.files
          ? response.files.map((f: any) => f.fileUrl)
          : response.fileUrl
          ? [response.fileUrl]
          : [];

        if (fileUrls.length > 0) {
          const newFiles = fileUrls.map((url, i) => ({
            name:
              response.files?.[i]?.originalName ??
              selectedFiles[i]?.name ??
              `파일 ${i + 1}`,
            fileUrl: url,
            fileSize: response.files?.[i]?.fileSize ?? selectedFiles[i]?.size,
            mimeType: response.files?.[i]?.mimeType ?? selectedFiles[i]?.type,
          }));
          setUploadedFiles((prev) => [...prev, ...newFiles]);
          toast.success('파일이 업로드되었습니다.');
        } else {
          toast.error('업로드된 파일 URL을 가져올 수 없습니다.');
        }
      } else {
        toast.error('파일 업로드에 실패했습니다.');
      }
    } catch (err: any) {
      toast.error(err?.message ?? '파일 업로드 중 오류가 발생했습니다.');
    } finally {
      ...
      if (isEditMode && initialData) {
        ...
        const res = await updateJobApplication({
          ...
          files: uploadedFiles.map((f) => ({
            fileType: f.fileType ?? 'ATTACHMENT',
            fileUrl: f.fileUrl,
            fileName: f.name,
            fileSize: f.fileSize,
            mimeType: f.mimeType,
              ...
              <div className="rounded-lg border bg-card divide-y max-h-48 overflow-y-auto">
                {savedAddresses.map((addr) => (
                  <button
                    ...
                    onClick={() => {
                      setValue('roadAddress', addr.roadFullAddr);
                      setValue('zipCode', addr.zipNo);
                      if (addr.addrDetail) {
                        setValue('addressDetail', addr.addrDetail);
                      }
                      setShowSavedAddresses(false);
                      toast.success('저장된 주소가 적용되었습니다.');
                    }}
                  >
                    <p className="font-medium">{addr.roadFullAddr}</p>
                    {addr.addrDetail && (
