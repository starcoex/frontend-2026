...
async function callOcrRest(imageBase64: string): Promise<OcrResult> {
  ...
  if (!res.ok) throw new Error(`REST OCR 실패: ${res.status}`);

  const json = await res.json();
  if (!json.success) throw new Error(json.error ?? 'OCR 처리 실패');
  return json.result as OcrResult;
}
...
export function DriverLicenseVerify({
  driverId,
  currentLicenseNumber,
  onVerified,
  preVerifyMode = false,
}: Props) {
  const { verifyDriverLicense, ocrDriverLicense } = useDelivery();
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      // 1차: REST API
      try {
        result = await callOcrRest(imageBase64);
      } catch (restErr) {
        console.warn('[OCR] REST 실패, GraphQL로 재시도:', restErr);
        toast.info('REST OCR 실패, GraphQL 방식으로 재시도합니다...');

        // 2차: GraphQL 폴백
        const gqlRes = await ocrDriverLicense({ imageBase64 });
        if (gqlRes.success && gqlRes.data?.result) {
          result = gqlRes.data.result as OcrResult;
        } else {
          throw new Error(gqlRes.data?.error?.message ?? 'GraphQL OCR 실패');
        }
      }
      ...
      if (result) {
        setForm((prev) => ({
          ...
          birthY: result!.birthY ?? prev.birthY,
          birthM: result!.birthM ?? prev.birthM,
          birthD: result!.birthD ?? prev.birthD,
          licenNo0: result!.licenNo0 ?? prev.licenNo0,
          licenNo1: result!.licenNo1 ?? prev.licenNo1,
          licenNo2: result!.licenNo2 ?? prev.licenNo2,
          licenNo3: result!.licenNo3 ?? prev.licenNo3,
          ghostNum: result!.ghostNum ?? prev.ghostNum,
        }));
        if (result.ghostNum) setHasGhostNum(true);
        setOcrDone(true);
        toast.success('OCR 추출 완료! 내용을 확인 후 진위 확인을 진행하세요.');
      }
    } catch (err) {
      toast.error('OCR 처리에 실패했습니다. 직접 입력해주세요.');
    } finally {
  ...
  const handleVerify = async () => {
    ...
    try {
      const input: VerifyDriverLicenseInput = {
        ...
        // ✅ driverId가 0이거나 없으면 undefined로 전달
        // → 백엔드에서 DB 저장 없이 진위확인만 수행 (preVerifyMode 지원)
  ...
  // ✅ 이미 인증된 경우 — 인증 완료 화면 표시
  if (currentLicenseNumber && !preVerifyMode) {
    return (
      <Card className="border-green-200">
        ...
        <CardContent className="space-y-4">
          ...
          <p className="text-muted-foreground text-xs">
            면허 정보를 재확인하거나 갱신이 필요한 경우 아래에서 다시
            진행하세요.
          </p>
          {/* 재확인이 필요한 경우 폼 보여주기 */}
          <Button
    ...
    <Card>
      ...
      <CardContent className="space-y-6">
        ...
        <div className="space-y-3">
          ...
          <p className="text-muted-foreground text-xs">
            이미지를 업로드하면 정보를 자동 추출합니다.
            <span className="ml-1 text-blue-600">
              (REST → 실패 시 GraphQL 자동 재시도)
            </span>
          </p>
          <div className="flex items-center gap-2">
            <input
