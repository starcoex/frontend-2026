...
interface AutoFocusInputProps {
  ...
  maxLength: number;
  nextRef?: React.RefObject<HTMLInputElement | null>; // ✅ React 19 타입
  placeholder?: string;
  className?: string;
  id?: string;
}
...
export function DriverGqlLicenseVerify({
  driverId,
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      // ✅ File → Base64 변환 후 GraphQL mutation 호출
      ...
      const res = await ocrDriverLicense({ imageBase64 });
      ...
      if (res.success && res.data?.result) {
        ...
        toast.error(res.data?.error?.message ?? 'OCR 처리에 실패했습니다.');
      }
    } catch {
      ...
      <CardContent className="space-y-6">
        ...
        <div className="space-y-3">
          ...
          <p className="text-muted-foreground text-xs">
            이미지를 업로드하면 정보를 자동으로 추출합니다. (GraphQL OCR)
          </p>
          <div className="flex items-center gap-2">
            <input
              ref={fileInputRef}
