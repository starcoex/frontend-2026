...
const FormSchema = z.object({
  ...
  bodyType: z.enum(['PASSENGER', 'SUV', 'VAN', 'TRUCK'] as const, {
    message: '차체 유형을 선택하세요.',
  }),
  sizeGrade: z.enum(['S', 'M', 'L', 'XL', 'XXL'] as const, {
    message: '사이즈 등급을 선택하세요.',
  }),
  price: z.number({ message: '가격을 입력하세요.' }).min(0),
  unit: z.string(),
...
export default function CarCarePriceCreatePage() {
  const navigate = useNavigate();
  const { createCarCarePrice } = useCarCare();
  ...
  const onSubmit = async (data: FormValues) => {
    const res = await createCarCarePrice({
    ...
    if (res.success) {
      toast.success('세차 가격이 추가되었습니다.');
      navigate('/admin/car-care/prices');
    } else {
      toast.error(res.error?.message ?? '가격 추가에 실패했습니다.');
    }
  };
  ...
  return (
    <>
      <PageHead
        title={`세차 가격 추가 - ${COMPANY_INFO.name}`}
        description="새로운 세차 가격 정책을 추가하세요."
        keywords={['세차 가격 추가', COMPANY_INFO.name]}
        og={{
          title: `세차 가격 추가 - ${COMPANY_INFO.name}`,
          description: '세차 가격 정책 추가',
