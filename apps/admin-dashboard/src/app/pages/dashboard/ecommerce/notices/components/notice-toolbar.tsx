...
export function NoticeToolbar({ table, onRefresh }: NoticeToolbarProps) {
  const isFiltered = table.getState().columnFilters.length > 0;
  const { bulkDeleteNotices } = useNotices();

  const handleBulkDelete = async (ids: number[]) => {
    return bulkDeleteNotices({ ids });
  };
    ...
    <div className="space-y-3">
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <BulkDeleteToolbar
          ...
          itemLabel="공지"
          deleteDescription="선택한 공지가 삭제됩니다. 이 작업은 되돌릴 수 없습니다."
        />
